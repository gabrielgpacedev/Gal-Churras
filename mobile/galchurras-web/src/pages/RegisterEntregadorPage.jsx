...
// Formulário individual — Entregador.
export default function RegisterEntregadorPage() {
  const { submit, error, setError, loading } = useRegister();
  const [f, setF] = useState({
    nome: '', documento: '', dataNascimento: '', genero: '', placa: '',
  ...
  const onSubmit = (e) => {
    ...
    if (!f.placa.trim()) return setError('Informe a placa do veículo.');
    ...
    submit({
      ...
      senha: f.senha,
      tipoUsuario: ROLES.ENTREGADOR,
    });
  };

  return (
    <RegisterLayout
      title={<>Cadastro de <em>entregador</em></>}
      subtitle="Cadastre-se para receber e realizar entregas."
    >
      <form onSubmit={onSubmit} className="login-form">
        ...
        <div className="field-group">
          ...
          <input className="reg-input" type="text" inputMode="numeric" placeholder="000.000.000-00" value={f.documento} onChange={set('documento')} />
        </div>
        ...
        <div className="field-group">
          ...
          <input className="reg-input" type="text" placeholder="ABC-1D23" value={f.placa} onChange={set('placa')} />
        </div>
